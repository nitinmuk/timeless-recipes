...
/**
 * generates pre-signed url to access image for 24 hrs
 * if image key is defined otherwise return default image url
 * @param {s3 object key} imageKey
 */
const generateImageUrlForClient = imageKey => {
  if (imageKey) {
...
/**
 * if an image file was sent then upload it to amazon S3 bucket
 * and return fileName generated by multer as image key to save in database
 * if not file was sent then retun null
...
/**
 * randomly select one recipe with image out of 20 and send
 * its details as recipe of the day.
 */
const getRecipeOfTheDay = async () => {
  try {
    ...
    const selectedID = Math.floor(
      Math.random() * findRecipesWithPictures.length
    );
    const recipeOfTheDay = findRecipesWithPictures[selectedID];
    const recipeImageUrl = generateImageUrlForClient(recipeOfTheDay.imageUrl);
    return {
      id: recipeOfTheDay.id,
      title: recipeOfTheDay.title,
      preparationTime: recipeOfTheDay.preparationTime,
      imageUrl: recipeImageUrl
    };
  } catch (error) {
    console.log(
      `Error ocurred while retreiving recipe of the day. Detailed error: ${error.stack}`
    );
    return;
  }
};
...
/**
 * deleltes input recipe id image from S3 storage.
